...
new_init.params['fc8_voc12'][0].reshape(21, 1024, 1, 1)
...
new_init.params['fc8_voc12'][1].reshape(1, 1, 1, 21)
new_init.params['fc8_voc12'][1].data.flat = temp.flat
new_init.reshape()


model = './voc12/config/vgg128_noup/deploy2.prototxt'
weights = './voc12/model/vgg128_noup/init4.caffemodel'
...
"""
model = './voc12/config/vgg128_noup/deploy2.prototxt'
weights = './voc12/model/vgg128_noup/test/init.caffemodel'
net_test = caffe.Net(model, weights)
new_init = caffe.Net(model, weights)
lists = new_init.params.keys()
for l in lists:
	x = new_init.params[l][0].data[0]
  	y = net_test.params[l][0].data[0]
  	print x
  	print y
  	print np.array_equal(x,y)
  	print "\n"
