...
\begin{figure}[ht]
  ...
  \end{tabular}
  \caption{Score map (input before softmax function) and belief map (output after softmax function) for Aeroplane. We show the score (1st row) and belief (2nd row) maps after each iteration of the employed inference algorithm. The initial value (at iteration 0) is the DCNN score/belief map.
    }
...
\begin{figure}
  ...
  \caption{Model Illustration. The coarse score map from Deep Convolutional Neural Network (with fully convolutional layers) is upsampled by bi-linear
    interpolation. A fully connected CRF is applied to refine the segmentation result.}
