...
\begin{figure}[ht]
  ...
  \end{tabular}
  \caption{Score map (output of last layer) and belief map (output after softmax function) for Aeroplane. First row: the score map at each iteration. Second row: the belief map at each iteration. The initial value (at iteration 0) is the DCNN score/belief map.
    }
