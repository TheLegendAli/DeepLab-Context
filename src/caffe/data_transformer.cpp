...
void DataTransformer<Dtype>::Transform(const cv::Mat& cv_img,
  ...
  /* jay add for segmentation input
   *
   *
   */
  /*
...
void DataTransformer<Dtype>::TransformSegAndPad(const cv::Mat& cv_seg,
  ...
  CHECK(cv_seg.depth() == CV_8U) << "Image data type must be unsigned byte";
  ...
  for (int h = 0; h < height; ++h) {
	...
	if (w < img_width && ptr != NULL) {
	  Dtype pixel = static_cast<Dtype>(ptr[img_index++]);
	  transformed_data[top_index] = pixel;
	} else {
	  Dtype mask_val = 255;
	  transformed_data[top_index] = mask_val;
	}
      }
    }
  }
...
void DataTransformer<Dtype>::TransformAndPad(const cv::Mat& cv_img,
  ...
  for (int h = 0; h < height; ++h) {
	...
	if (w < img_width && ptr != NULL) {
	  ...
	  // outside image domain, just assign zero
...
void DataTransformer<Dtype>::TransformImgAndSeg(const std::vector<cv::Mat>& cv_img_seg, Blob<Dtype>* transformed_data_blob, Blob<Dtype>* transformed_label_blob) {
  ...
  for (int h = 0; h < data_height; ++h) {
	...
	// outside image domain, just assign "ambiguous" label
	Dtype mask_val = 255;
	transformed_label[top_index] = mask_val;
      }      
    }
  }
  */
